...
public class BugBoxMenu extends AbstractContainerMenu {
    public final Container container;

    public BugBoxMenu(int i, Inventory inventory) {
        this(i, inventory, new SimpleContainer(54));
    }

    public BugBoxMenu(int i, Inventory inventory, Container container) {
        super(IttyBittyMenuType.BUGBOX, i);
        int m;
        int l;
        ...
        for (l = 0; l < 6; ++l) {
            for (m = 0; m < 9; ++m) {
                this.addSlot(new BugBoxSlot(container, m + l * 9, 8 + m * 18, 18 + l * 18));
            }
        }
        for (l = 0; l < 3; ++l) {
            for (m = 0; m < 9; ++m) {
